...
export const deployPoll = async ({
  ...
  try {
    ...
    const result: any = {};
    result.pollId = pollId.toString();
    result.pollAddr = pollAddr;
    result.messageProcessorContractAddress = messageProcessorContractAddress;
    result.tallyContractAddress = tallyContractAddress;
    result.subsidyContractAddress = subsidyContractAddress;
    logGreen(quiet, info(`Poll ID: ${pollId.toString()}`));
    logGreen(quiet, info(`Poll contract: ${pollAddr}`));
    logGreen(
      quiet,
      info(`Message Processor contract: ${messageProcessorContractAddress}`),
    );

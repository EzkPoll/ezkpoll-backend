...
export const verify = async ({
  ...
  if (!(await contractExists(signer.provider!, tallyContractAddress))) {
    logError(
      `Error: there is no Tally contract deployed at ${tallyContractAddress}.`,
    );
  }
  ...
  if (!(await contractExists(signer.provider!, maciContractAddress))) {
    logError(
      `Error: there is no MACI contract deployed at ${maciContractAddress}.`,
    );
  }
  ...
  if (subsidyEnabled) {
    ...
    if (!(await contractExists(signer.provider!, subsidyContractAddress))) {
      logError(
        `Error: there is no Subsidy contract deployed at ${subsidyContractAddress}.`,
      );
    }
  }
  ...
  const subsidyContract = subsidyEnabled
    ? SubsidyFactory.connect(subsidyContractAddress, signer)
    : undefined;
  ...
  logYellow(
    quiet,
    info(`on-chain tally commitment: ${onChainTallyCommitment.toString(16)}`),
  );

  // check the results commitment
  const validResultsCommitment =
    tallyResults.newTallyCommitment.match(/0x[a-fA-F0-9]+/);
  ...
  if (useQv) {
    ...
    if (isValid) {
      logGreen(
        quiet,
        success(
          'The on-chain verification of total spent voice credits passed.',
        ),
      );
    } else {
      logError(
    ...
    if (failedSpentCredits.length === 0) {
      logGreen(
        quiet,
        success(
          'The on-chain verification of per vote option spent voice credits passed',
        ),
      );
    } else {
      logError(
    ...
    if (failedPerVOSpentCredits.length === 0) {
      logGreen(
        quiet,
        success('The on-chain verification of tally results passed'),
      );
    } else {
      logError(
    ...
    // compute newTallyCommitment
    const newTallyCommitment = hashLeftRight(
      newResultsCommitment,
      newSpentVoiceCreditsCommitment,
    );
  ...
  if (subsidyEnabled && subsidyData && subsidyContract !== undefined) {
    ...
    logYellow(
      quiet,
      info(
        `on-chain subsidy commitment: ${onChainSubsidyCommitment.toString(16)}`,
      ),
    );

    // check the results commitment
    const validResultsSubsidyCommitment =
